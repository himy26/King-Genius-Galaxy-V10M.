        ...
        def _scan():
            ...
            try:
                ...
                try:
                    ...
                    self.log(f"⚡ FASTBOOT DETECTED: {fb.split(':')[1].strip()}", "SUCCESS")
    ...
    def execute_command(self):
        ...
        try:
            if brand == "samsung":
                ...
                subprocess.run(["adb", "reboot", "download"], capture_output=True)
            
            elif brand == "huawei" or brand == "infinix_tecno":
                self.log("🔐 Sending FRP Wipe Command (Fastboot)...", "INFO")
                # مسح قسم البيانات الأمنية (أمر حقيقي لمعظم أجهزة MTK/Kirin في وضع Fastboot)
                subprocess.run(["fastboot", "erase", "config"], capture_output=True)
                subprocess.run(["fastboot", "erase", "frp"], capture_output=True)
                subprocess.run(["fastboot", "reboot"], capture_output=True)
            
            elif brand == "motorola_vivo":
                self.log("🔓 Fetching OEM Unlock Keys...", "INFO")
                subprocess.run(["fastboot", "getvar", "all"], capture_output=True)
